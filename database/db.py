...
def get_db_connection():
    ...
    If MySQL connection fails or is unavailable, falls back to SQLite transparently
    to guarantee zero-friction immediate execution while maintaining server-side persistence.
    """
    global DB_TYPE
    ...
    try:
        ...
        # Fallback to SQLite persistent database file
        DB_TYPE = 'sqlite'
...
def init_db():
    """Initializes schema and seeds if not already created."""
    ...
    if db_type == 'sqlite':
        ...
        CREATE TABLE IF NOT EXISTS users (
            ...
            gender TEXT,
            category TEXT,
            income_annual REAL DEFAULT 0.00,
            occupation TEXT,
            state TEXT DEFAULT 'Pan-India',
            district TEXT,
            address TEXT,
            dbt_bank_account TEXT,
            ifsc_code TEXT,
            bank_name TEXT,
            dbt_linked INTEGER DEFAULT 1,
            profile_completed INTEGER DEFAULT 85,
        ...
        CREATE TABLE IF NOT EXISTS schemes (
            ...
            benefit_type TEXT DEFAULT 'Direct Benefit Transfer (DBT)',
            eligibility_min_age INTEGER DEFAULT 0,
            eligibility_max_age INTEGER DEFAULT 120,
            eligibility_max_income REAL DEFAULT 1000000.00,
        ...
        CREATE TABLE IF NOT EXISTS applications (
            ...
            disbursed_amount REAL DEFAULT 0.00,
        ...
        CREATE TABLE IF NOT EXISTS reminders (
            ...
            reminder_type TEXT DEFAULT 'disbursement',
            description TEXT,
        ...
        # Migration: add 'attachment' column for installs created before this feature existed.
        # Wrapped in try/except since both SQLite and MySQL error if the column already exists,
        # and this block runs on every app startup.
        try:
            if db_type == 'mysql':
                cursor.execute("ALTER TABLE grievances ADD COLUMN attachment VARCHAR(255)")
            else:
                cursor.execute("ALTER TABLE grievances ADD COLUMN attachment TEXT")
            conn.commit()
        except Exception:
        ...
        if cursor.fetchone()[0] == 0:
            ...
            pwd_citizen = generate_password_hash('citizen123')
            
            # Seed users
            cursor.execute('''
            INSERT INTO users (id, aadhaar_no, full_name, email, mobile, password_hash, role, dob, gender, category, income_annual, occupation, state, district, dbt_bank_account, ifsc_code, bank_name)
            VALUES 
            (1, '999988887777', 'Rajesh Verma (Nodal Officer)', 'admin@saarthi.gov.in', '9876543210', ?, 'admin', '1980-05-15', 'Male', 'General', 1200000.00, 'Government Service', 'Delhi', 'New Delhi', '918273645019', 'SBIN0001001', 'State Bank of India'),
            (2, '234567890123', 'Ramesh Kumar Patel', 'ramesh.patel@gmail.com', '9812345678', ?, 'citizen', '1988-08-20', 'Male', 'OBC', 180000.00, 'Farmer', 'Uttar Pradesh', 'Varanasi', '501002349182', 'HDFC0000123', 'HDFC Bank'),
            (3, '345678901234', 'Priya Sharma', 'priya.sharma@gmail.com', '9876512345', ?, 'citizen', '1995-12-10', 'Female', 'General', 240000.00, 'Entrepreneur', 'Maharashtra', 'Pune', '309281746501', 'ICIC0000999', 'ICICI Bank'),
            (4, '456789012345', 'Sunita Devi', 'sunita.devi@gmail.com', '9823456789', ?, 'citizen', '1990-03-25', 'Female', 'SC', 95000.00, 'Artisan', 'Bihar', 'Patna', '601293847561', 'PUNB0000555', 'Punjab National Bank')
            ''', (pwd_admin, pwd_citizen, pwd_citizen, pwd_citizen))
            
            # Seed schemes -- 20 total (7 original + 13 new).
            # Categories normalized to exactly match the 8 homepage category
            # filters: Agriculture, Housing, Healthcare, Education, Women,
            # Employment, Pension, Entrepreneurship.
            schemes_data = [
                (1, 'PM-KISAN', 'Pradhan Mantri Kisan Samman Nidhi', 'Ministry of Agriculture & Farmers Welfare', 'Agriculture', 6000.00, 'Direct Benefit Transfer (DBT)', 18, 80, 400000.00, 'All', 'All', 'Farmer', 'Financial assistance of Rs. 6,000 per year provided to all landholding farmer families across India in three equal installments.', 'Rs. 6,000/year credited directly into bank account in 3 installments of Rs. 2,000.', 'Aadhaar Card, Land Holding Certificate, Bank Passbook', '2026-12-31', 'active'),
            ...
            cursor.executemany('''
            INSERT INTO schemes (id, code, title, ministry, category, benefit_amount, benefit_type, eligibility_min_age, eligibility_max_age, eligibility_max_income, gender_target, target_category, target_occupation, description, benefits_summary, documents_required, deadline, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', schemes_data)

            # Seed Applications -- 5 original + 8 new (covering the new schemes)
            apps_data = [
                ...
                (2, 'SRT-2026-45109', 3, 4, 'approved', '2026-06-15 11:45:00', '2026-06-18 09:30:00', '2026-06-22 15:10:00', '2026-07-01 12:00:00', None, 'MUDRA project proposal approved by Nodal Credit Committee.', 'DBT-PENDING-STAGE', 0.00),
                (3, 'SRT-2026-78210', 3, 5, 'field_verification', '2026-07-05 14:20:00', '2026-07-08 10:15:00', None, None, None, 'Academic records verified. District Education Officer site visit scheduled.', None, 0.00),
                (4, 'SRT-2026-12904', 4, 3, 'under_scrutiny', '2026-07-20 09:10:00', None, None, None, None, 'Document scrutiny in progress by Health Department Officer.', None, 0.00),
                (5, 'SRT-2026-66389', 2, 2, 'submitted', '2026-07-25 16:50:00', None, None, None, None, 'Application received. Pending initial officer assignment.', None, 0.00),
                (6, 'SRT-2026-31207', 2, 8, 'approved', '2026-06-10 09:00:00', '2026-06-12 10:00:00', '2026-06-15 13:30:00', '2026-06-20 11:00:00', None, 'Crop insurance policy approved for current Kharif season.', 'DBT-PENDING-STAGE', 0.00),
                ...
                (8, 'SRT-2026-64029', 4, 12, 'submitted', '2026-07-28 12:15:00', None, None, None, None, 'JSY application received, awaiting ASHA worker verification.', None, 0.00),
                (9, 'SRT-2026-70933', 2, 13, 'approved', '2026-04-01 08:00:00', '2026-04-02 09:00:00', '2026-04-03 10:00:00', '2026-04-04 11:00:00', None, 'PMSBY accident cover activated for the policy year.', 'DBT-PENDING-STAGE', 0.00),
                (10, 'SRT-2026-81456', 3, 14, 'field_verification', '2026-07-01 10:00:00', '2026-07-04 09:30:00', None, None, None, 'PM-YASASVI eligibility documents under caste category verification.', None, 0.00),
                ...
                (12, 'SRT-2026-11732', 2, 18, 'under_scrutiny', '2026-07-15 09:00:00', None, None, None, None, 'Atal Pension Yojana enrolment under bank verification.', None, 0.00),
                (13, 'SRT-2026-20984', 3, 20, 'rejected', '2026-05-20 09:00:00', '2026-05-22 10:00:00', '2026-05-25 11:00:00', None, None, 'Project report did not meet minimum viability criteria for the loan slab.', None, 0.00),
            ]
            cursor.executemany('''
            INSERT INTO applications (id, application_ref, user_id, scheme_id, status, applied_date, scrutiny_date, verification_date, approval_date, disbursement_date, remarks, dbt_transaction_id, disbursed_amount)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', apps_data)

            # Seed Documents -- original 5 + 3 new
            docs_data = [
            ...
            cursor.executemany('''
            INSERT INTO documents (user_id, application_id, doc_type, file_name, file_path, verification_status)
            VALUES (?, ?, ?, ?, ?, ?)
            ''', docs_data)

            # Seed Notifications & Reminders & Grievances
            cursor.executemany('''
            INSERT INTO notifications (user_id, title, message, type, is_read)
            VALUES (?, ?, ?, ?, ?)
            ''', [
                (2, 'DBT Payment Disbursed', 'Rs. 2,000 has been credited to your HDFC Bank account under PM-KISAN. Ref: DBT-RBI-90182746352', 'success', 0),
                (3, 'MUDRA Loan Approved', 'Your application SRT-2026-45109 for PM-MUDRA Yojana has been approved by the Nodal Officer.', 'success', 0),
                (3, 'Field Verification Alert', 'Inspector assigned for your National Scholarship application SRT-2026-78210.', 'info', 1),
                (4, 'Application Submitted', 'Your Ayushman Bharat application SRT-2026-12904 was received successfully.', 'info', 0),
                (2, 'Crop Insurance Approved', 'Your PMFBY application SRT-2026-31207 has been approved for the current Kharif season.', 'success', 0),
                (3, 'PMAY-G Assistance Disbursed', 'Rs. 1,30,000 has been credited to your account under PMAY-G. Ref: DBT-RBI-77281937465', 'success', 1),
                (4, 'PMMVY Installment Credited', 'Rs. 1,000 has been credited under Pradhan Mantri Matru Vandana Yojana. Ref: DBT-RBI-65423190872', 'success', 0),
                (3, 'Stand-Up India Application Rejected', 'Your application SRT-2026-20984 was rejected. Reason: project report did not meet minimum viability criteria.', 'danger', 0),
            ])
            ...
            cursor.executemany('''
            INSERT INTO grievances (ticket_no, user_id, application_id, subject, description, status, response)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', [
...
def query_db(query, args=(), one=False):
    """Executes SELECT queries returning dictionaries."""
    conn, db_type = get_db_connection()
    cursor = conn.cursor()
